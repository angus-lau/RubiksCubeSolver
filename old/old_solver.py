...
import old.checker as check
import old.permutations as perm
...
ALLOWED_MOVES_IDA = ['U','D','R2','L2','F2','B2']
...
# Create IDA* search function
def ida_search_phase1(start_state, goal_state, allowed_moves):
    # Track visited with a set for O(1) lookup, and prevent duplicates
    visited = set()
    # Track the current state and moves taken to reach it
    stack, path = [start_state, []]
    while stack:
        curr_state = stack.pop()
        
        # If current state matches goal state, return path
        if curr_state == goal_state:
            return path
        
        visited.add(curr_state)

        # Apply every move to the current state
        for move in allowed_moves:
            new_state = apply_move(curr_state, move)
            # If the state isnt in visited, add it to the stack
